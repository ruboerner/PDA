...
Anticipating the result, we look for a solution which is the sum of a gyration at $\omega_c$ and a steady drift $\mathbf{v}_E$. Since we are interested in finding an expression for $v_E$, we take out the gyratory motion by averaging over a cycle. The $v_x$ component then gives $\bar{v}_x = 0$.[^tutorial-1] In the $v_y$ component, the oscillating term $\ddot{v}_y$ clearly averages to zero, and we have

[^tutorial-1]: I am kind of lost here. Probably the logic here is that first we assume the solution has an additional steady drift, so we let $\ddot{v}_y = 0$? Check Y.Y's note.
...
Polarization drift:[^tutorial-2]

[^tutorial-2]: In common test particle models, we assume static EM fields, so the polarization drift as well as adiabatic heating is not present. However, it is easily achieveable here in this package.
