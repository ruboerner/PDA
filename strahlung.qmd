...
println("Solarkonstante: S=$(round(S, digits=2))")
```

Der über Satellitenmessungen betimmte Wert beträgt $S=1361$ $W m^{-2}$.

Die Verteilung von Kontinenten und Ozeanen auf der Erdoberfläche führt zur Absorption und unvollständigen Rückstrahlung der Sonneneinstrahlung. Das Rückstrahlvermögen oder die Albedo der Erde beträgt 0.3, damit beträgt die Absorption 0.7.
...
```{julia}
#| code-fold: true
println("$(round(S * pi * R_E^2 * 1e-15, digits=3)) PW.")
```
...
println("Oberflächentemperatur: T = $(round(T, digits=1)) °C")
```
...
println("Effektive Temperatur der Sonne: T = $(round(T, digits=2)) K.")
```
